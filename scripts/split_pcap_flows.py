#!/usr/bin/env python3
"""
Split all PCAPs under a directory into per-flow PCAPs (one file per 5-tuple).
...
from earlybird.pcap_processing import discover_pcaps, split_pcap_into_flows


def _process_one(pcap_path: Path, output_dir: Path, input_root: Path) -> dict:
    """Worker: process one PCAP (top-level for pickling)."""
    result = split_pcap_into_flows(pcap_path, output_dir, input_root=input_root)
...
def main() -> None:
    ...
    default_output = WORKSPACE_ROOT / "data" / "ucsb_data_flows"

    ap = argparse.ArgumentParser(description="Split PCAPs into per-flow PCAPs.")
    ...
    print(f"Found {len(pcaps)} PCAP(s), processing in parallel (jobs={n_jobs})...\n")
    ...
    with ProcessPoolExecutor(max_workers=n_jobs) as executor:
        ...
        for future in as_completed(future_to_info):
            ...
            print(
                ...
                f"{result['flow_count']} flows, {result['packet_count']} packets, "
                f"{result['skipped_count']} skipped  [{done}/{len(pcaps)} done]"
            )

    print(
        f"\nDone: {len(results)} PCAPs -> {sum(r['flow_count'] for r in results)} flow files "
